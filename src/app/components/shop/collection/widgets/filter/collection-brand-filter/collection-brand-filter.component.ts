...
export class CollectionBrandFilterComponent {
  ...
  ngOnInit() {
    ...
    //     },
    //   });
    this.fetchBrandSidebarFiltersAPI();
  }

  fetchBrandSidebarFiltersAPI() {
    this.servicesService.getMockBrandsSidebar().subscribe((response: any) => {
  ...
  applyFilter(event: Event) {
    ...
    if ((<HTMLInputElement>event?.target)?.checked)
      this.selectedBrands.push((<HTMLInputElement>event?.target)?.value);
