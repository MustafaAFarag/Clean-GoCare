...
export class CollectionProductsComponent {
  ...
  ngOnInit() {
    ...
    //   console.log("All Products: ", this.productsArray); // Log before filtering
    //   this.total = res.total;
    // });
    this.setPage();
    this.fetchCollectionProductAPI();
  }

  // COLLECTION PRODUCT API

  fetchCollectionProductAPI() {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    console.log("Filter Applied: ", this.filter); // Log the current filter params
    ...
    this.route.queryParams.subscribe(() => {
      ...
      if (this.filter["layout"] == "collection_product_infinite_scroll") {
        this.moreProductSubscription = this.moreProduct$.subscribe(
          (product) => {
      ...
      console.log("Filtered Products: ", this.productsArray); // Log after filtering
  ...
  onScroll(value: number) {
    if (this.products != this.total_product) {
      ...
      this.store
        .dispatch(new GetMoreProduct(this.scrollFilter, true))
        .subscribe({
          complete: () => {
